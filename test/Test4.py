...
class TestGetActoresRolesByTitle(unittest.TestCase):
    ...
    def test_get_actor_roles_by_title(self):
        ...
        name_df = self.spark.createDataFrame(name_data, ["nconst", "primaryName"])

        title_principals_data = [("1", "actor", "1", "Dad"), ("2", "director", "2", ""), ("3", "actress", "3", "Son")]
        title_principals = self.spark.createDataFrame(title_principals_data, ["nconst", "category", "tconst", "characters"])

        title_akas_data = [("1", "Movie 1"), ("2", "Movie 2"), ("3", "Movie 3")]
        title_akas = self.spark.createDataFrame(title_akas_data, ["tconst", "title"])
